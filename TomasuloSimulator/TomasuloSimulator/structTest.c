...
BTBRow* createBTBRow(int* Key){
	BTBRow* res = (BTBRow*)malloc(sizeof(RSRow));
	res->Key = Key;
	res->FetchedPC = -1;
	res->PredictPC = -1;
	res->BitOfExtra = -1;
...
void main(){
	...
	int i;
	for(i = 0; i < UpperSizeOfROB; i++){
		dequeueCircular(ROB);
		printf("Test element num is:%d\n", getCountCircularQueue(ROB));
	}
//}
...
//}
        ...
        int c3 = 3;
        int c4 = 4;
        BTBRow* BTBRow0 = createBTBRow(&c0);
        printf("value = %d,KeyAddress = %d\n", *(BTBRow0->Key), BTBRow0->Key);
        BTBRow* BTBRow1 = createBTBRow(&c1);
        BTBRow* BTBRow2 = createBTBRow(&c2);
        BTBRow* BTBRow3 = createBTBRow(&c3);
        printf("value = %d,KeyAddress = %d\n", *(BTBRow3->Key), BTBRow3->Key);
        printf("BTBRow0 value = %d\n", *(BTBRow0->Key));
        BTBRow* BTBRow4 = createBTBRow(&c4);
        DictionaryBTBRow* BTBTable = createDictionaryBTBRow(getHashCode, compareValues);
        DictionaryBTBRowEntry* Oper_BTBRow = createDictionaryBTBRowEntry(BTBTable, BTBRow4->Key, BTBRow4);

	printf("BTBRow0->Key value = %d, BTBRow0->Key = %d\n", *(BTBRow0->Key), BTBRow0->Key);
        printf("BTBRow3->Key value = %d, BTBRow3->Key = %d\n", *(BTBRow3->Key), BTBRow3->Key);
        printf("RS address = %d\n", BTBTable);
        addDictionaryBTBRowEntry(BTBTable, BTBRow0->Key, BTBRow0);
        addDictionaryBTBRowEntry(BTBTable, BTBRow1->Key, BTBRow1);
        addDictionaryBTBRowEntry(BTBTable, BTBRow2->Key, BTBRow2);
        addDictionaryBTBRowEntry(BTBTable, BTBRow3->Key, BTBRow3);
        addDictionaryBTBRowEntry(BTBTable, BTBRow4->Key, BTBRow4);

        printf("81\n");
	
	Oper_BTBRow = getValueChainByDictionaryBTBRowKey(BTBTable, (BTBRow0->Key));
        printf("Before get, Oper_BTBRow->value->Key is %d, and BTBRow0 FetchedPC is %d, and BTBRow0 Qj is %d\n", *(Oper_BTBRow->value->Key), BTBRow0->FetchedPC, BTBRow0->BitOfExtra);
        Oper_BTBRow->value->FetchedPC = 1001;
        Oper_BTBRow->value->BitOfExtra = 2;
	printf("After get, Oper_BTBRow->value->Key is %d, and BTBRow0 FetchedPC is %d, and BTBRow0 Qj is %d\n", *(Oper_BTBRow->value->Key), BTBRow0->FetchedPC, BTBRow0->BitOfExtra);	
	Oper_BTBRow = getValueChainByDictionaryBTBRowKey(BTBTable, (BTBRow3->Key));
        printf("%d\n", *(Oper_BTBRow->value->Key));
        removeDictionaryBTBRowEntriesByKey(BTBTable, BTBRow3->Key);
        Oper_BTBRow = getValueChainByDictionaryBTBRowKey(BTBTable, (BTBRow4->Key));
	Oper_BTBRow = getValueChainByDictionaryBTBRowKey(BTBTable, (BTBRow3->Key));
        printf("%d\n", *(Oper_BTBRow->value->Key));
